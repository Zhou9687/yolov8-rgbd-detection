...
def parse_args():
    ...
    # 必需参数
    parser.add_argument("--data", type=str, default="datasets/tennis-yolo/tennis-yolo.yaml", help="数据集配置文件路径")
    parser.add_argument("--model", type=str, default="yolov8_4ch_direct.pt", help="4通道预训练模型路径")
    ...
    parser.add_argument("--device", type=str, default="cuda:0", help="训练设备 (默认: cuda:0, 可选: cpu)")

    # 输出参数
    parser.add_argument("--name", type=str, default="train_rgbd", help="实验名称 (默认: train_rgbd)")
    parser.add_argument("--project", type=str, default="runs/detect", help="项目保存目录 (默认: runs/detect)")

    # 训练配置
    parser.add_argument("--patience", type=int, default=50, help="早停耐心值 (默认: 50)")
    parser.add_argument("--workers", type=int, default=0, help="数据加载进程数 (Windows建议为0)")
    parser.add_argument("--cache", action="store_true", help="是否缓存数据 (默认: False)")
    parser.add_argument("--amp", action="store_true", help="是否启用AMP混合精度 (默认: False)")

    # 数据增强参数（RGBD建议禁用）
    parser.add_argument("--mosaic", type=float, default=0.0, help="Mosaic增强概率 (RGBD建议0.0)")
    parser.add_argument("--mixup", type=float, default=0.0, help="Mixup增强概率 (RGBD建议0.0)")
    parser.add_argument("--copy-paste", type=float, default=0.0, help="Copy-Paste增强概率 (RGBD建议0.0)")

    # 其他参数
    parser.add_argument("--no-plots", action="store_true", help="不生成训练可视化图表")
    parser.add_argument("--verbose", action="store_true", default=True, help="详细输出模式")
