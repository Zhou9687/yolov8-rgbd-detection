...
def train_4ch_fixed():
    ...
    if first_conv.weight.shape[1] != 4:
        # 手动扩展输入通道数
        first_conv.weight = torch.nn.Parameter(
            torch.cat([first_conv.weight, torch.zeros_like(first_conv.weight[:, :1])], dim=1)
        )
    ...
    if trained_channels != 4:
        # 手动扩展输入通道数
        first_conv_trained.weight = torch.nn.Parameter(
            torch.cat([first_conv_trained.weight, torch.zeros_like(first_conv_trained.weight[:, :1])], dim=1)
        )
