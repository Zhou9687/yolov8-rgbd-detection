...
def parse_args():
    ...
    parser.add_argument("--model", type=str, required=True, help="训练好的模型路径 (必需)")
    parser.add_argument("--data", type=str, default="datasets/tennis-yolo/tennis-yolo.yaml", help="数据集配置文件路径")
    parser.add_argument(
        "--split", type=str, default="val", choices=["val", "test", "train"], help="验证数据集分割 (默认: val)"
    )
    parser.add_argument("--batch", type=int, default=1, help="批次大小 (默认: 1)")
    ...
    parser.add_argument("--save-json", action="store_true", help="保存COCO格式的结果JSON文件")
    parser.add_argument("--save-hybrid", action="store_true", help="保存标签+预测的混合结果")
